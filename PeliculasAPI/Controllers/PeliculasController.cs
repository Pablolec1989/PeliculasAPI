...
namespace PeliculasAPI.Controllers
{
    ...
    public class PeliculasController : CustomBaseController
    {
        ...
        public async Task<ActionResult<PeliculaDetallesDTO>> Get(int id)
        {
            ...
            //Verifico si la pelicula fue puntuada
            ...
            pelicula.VotoUsuario = votoUsuario;
        ...
        [HttpGet("filtrar")]
        [AllowAnonymous]
        public async Task<ActionResult<List<PeliculaDTO>>> Get([FromQuery] PeliculasFiltrarDTO peliculasFiltrarDTO)
        {
            ...
            if (!string.IsNullOrWhiteSpace(peliculasFiltrarDTO.Titulo))
            {
                peliculasQueryable = peliculasQueryable
                    .Where(p => p.Titulo
                    .Contains(peliculasFiltrarDTO.Titulo));
            }
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromForm] PeliculaCreationDTO peliculaCreationDTO)
        {
            var pelicula = mapper.Map<Pelicula>(peliculaCreationDTO);

            if (peliculaCreationDTO.Poster is not null)
            {
                var url = await almacenadorArchivos.Almacenar(contenedor, peliculaCreationDTO.Poster);
                pelicula.Poster = url;
            }
        ...
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Put(int id, [FromForm] PeliculaCreationDTO peliculaCreationDTO)
        {
            var pelicula = await context.Peliculas
            ...
            pelicula = mapper.Map(peliculaCreationDTO, pelicula);

            if (peliculaCreationDTO.Poster is not null)
            {
                pelicula.Poster = await almacenadorArchivos.Editar(pelicula.Poster,
                    contenedor, peliculaCreationDTO.Poster);
            }
